...
if [ -z "$(docker ps -q -f name=${container_name})" ]; then
    docker run -it --rm --publish $http_port:80 --publish $https_port:443 --publish-all --env-file $env_path --network=ecommerce --name $container_name --mount type=bind,src=${HOME}/.aspnet/https,dst=/https,readonly $image_name
fi
